...
func (a Application) PlaceOrder(order domain.Order) (domain.Order, error) {
	...
	if paymentErr != nil {
		...
		badReq.FieldViolations = append(badReq.FieldViolations, fieldErr) // populates with an actual payment detail

		orderStatus := status.New(codes.InvalidArgument, "order creation failed") // create root status
